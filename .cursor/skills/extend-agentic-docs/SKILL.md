---
name: extend-agentic-docs
description: Extends and maintains the agentic workspace documentation (WORKSPACE.md, ARCHITECTURE.md, .cursor/rules/, .cursor/skills/) as the implementation evolves. Use when an architectural decision is made, conventions change, a new feature area is added, or the agent needs to document a new pattern for future sessions.
---
...
- A new Cursor rule or skill is needed for a recurring pattern
...
| Convention or operating rule | `WORKSPACE.md` + relevant `.cursor/rules/*.mdc` |
...
| Recurring code pattern | New `.cursor/rules/*.mdc` or update an existing one |
| New agentic workflow | New `.cursor/skills/<name>/SKILL.md` |
...
WORKSPACE.md is the agent's "onboarding doc". Keep it accurate, not exhaustive.
...
1. Create `.cursor/rules/<descriptive-name>.mdc`
2. Set `globs` to the most specific pattern that applies
3. Set `alwaysApply: false` unless it truly applies everywhere
4. Keep it under 50 lines; use concrete `✅ / ❌` examples
...
3. Description must be third-person and include both WHAT and WHEN
4. Keep SKILL.md under 500 lines; link to supplementary files if needed
...
- [ ] No time-sensitive information (no "as of March 2026" type statements in code rules)
